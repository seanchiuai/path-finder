...
**Ported enhanced gamified dashboard from career-compass to main repo:**

**New Features:**
- **Career Overview Sidebar**: Displays median salary and 10-year growth outlook from `detailedPlan`
- **Learning Resources**: Expandable cards showing courses, books, certifications with descriptions and links
- **Project Ideas**: Expandable cards with difficulty levels, time estimates, and project guides
- **Networking Contacts**: Real LinkedIn profiles with titles, companies, and connection tips
- **Interview Prep**: Interview guides, practice resources, and question banks
- **Day in the Life Videos**: Embedded YouTube videos in sidebar (3 videos)
- **Grid Layout**: 2-column main content + 1-column sidebar for better information hierarchy

**Technical Changes:**
- **app/career/[id]/page.tsx**: Complete layout restructure to grid system
- Added expandable state management with chevron icons
- New icon imports: DollarSign, TrendingUp, ChevronDown, ChevronUp, ExternalLink
- Each track tab now displays relevant resources from `actionPlan.detailedPlan`
- Videos moved from bottom grid to sidebar with embedded iframes
- All resources have expandable descriptions with external link buttons

**Data Structure:**
- Leverages existing `detailedPlan` object stored in actionPlans table
- No backend changes required - data already generated by Python PlanDetailAgent
- Backward compatible: Shows "Data not available" if detailedPlan missing

**UX Improvements:**
- Expandable resource cards reduce visual clutter
- External link icons indicate clickable resources
- Difficulty badges color-coded (beginner/green, intermediate/yellow, advanced/red)
- Hover effects on resource cards for better interactivity
- Responsive grid layout adjusts for mobile, tablet, desktop

**Flow:** User views career detail → Sidebar shows salary/growth + videos → Each tab displays tasks + relevant resources → Click to expand resource → Click external link to access
