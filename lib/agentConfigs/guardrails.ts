...
import { GuardrailOutputZod, GuardrailOutput } from '@/types/realtime';

// Validator that calls the /api/realtime/responses endpoint to
...
export async function runGuardrailClassifier(
  ...
  const response = await fetch('/api/realtime/responses', {
    method: 'POST',
    headers: {
